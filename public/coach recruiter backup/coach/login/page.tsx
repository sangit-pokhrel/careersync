...
export default function CoachLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🔐 Step 1: Attempting coach login...');
      ...
      // Step 3: Check if user is career_coach
      if (user.role !== 'career_coach') {
        console.error('❌ Not a coach user:', user.role);
      ...
      console.log('✅ Step 5: Coach role verified');
      
      // Step 4: Store access token in cookie
      ...
      sessionStorage.setItem('coachToken', 'coachisloggedin');
      ...
      // Step 5: Redirect to coach dashboard
      console.log('🔄 Step 7: Redirecting to /coach');
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-green-50 to-blue-100">
      <div className="bg-white rounded-2xl p-8 shadow-lg max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mb-2">Career Coach Login</h1>
          <p className="text-sm text-gray-500">Access your coaching dashboard</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-green-600 text-white rounded-xl py-3 px-4 font-bold hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
